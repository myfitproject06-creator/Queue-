          ...
          {THEME_PRESETS.map((preset) => {
            ...
            return (
              <div
                ...
                className={`rounded-2xl border-2 p-4 sm:p-5 transition-all duration-200 ${
                  isActive
                    ? 'bg-slate-950/80 border-purple-500 shadow-xl shadow-purple-950/50 ring-1 ring-purple-500/40'
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-800/80">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-bold flex items-center justify-center gap-1.5 transition cursor-pointer flex-shrink-0 ${
                      isActive
                        ? 'bg-purple-950/60 text-purple-300 border border-purple-500/40 cursor-default opacity-80'
                        : 'bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white shadow-md shadow-purple-900/30 active:scale-95'
                    }`}
                  >
                    {isLoading ? (
                      <span className="animate-pulse">กำลังเปลี่ยน...</span>
                    ) : isActive ? (
                      <>
                        <Check className="w-4 h-4 text-purple-400" />
                        <span>เปิดใช้อยู่</span>
                      </>
                    ) : (
                      <>
                        <Sparkles className="w-4 h-4" />
                        <span>เลือกธีมนี้</span>
                      </>
                    )}
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                  {/* Left Side Preview */}
                  <div className="rounded-xl border border-slate-800 bg-slate-900/90 p-3 flex items-center gap-3">
